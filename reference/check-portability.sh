...
reference/test/moc-sync.test.sh 1 NOT frontmatter and NOT a conversion backlog: the hit is a grep for a caret-anchored 'derived:' line, asserting the provenance line of a GENERATED MOC — a file with no frontmatter block at all, so frontmatter.sh is the wrong tool rather than an unadopted one. Listed rather than reworded because dropping the anchor to slip past the detector is the one-rename evasion this guard's own header rejects, and it would weaken the assertion. NO BACKTICKS IN ANY REASON: this string is double-quoted, so a backtick is command substitution. The first version of this entry quoted the pattern in backticks, bash ran it, and the stored reason came out reading 'the hit is 0' while the guard still printed PASS
