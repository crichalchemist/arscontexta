...
reference/test/moc-sync.test.sh 1 NOT frontmatter and NOT a conversion backlog: the hit is `grep -c '^derived: [0-9]'` asserting the provenance line of a GENERATED MOC, a file that has no frontmatter block at all, so frontmatter.sh is the wrong tool rather than an unadopted one. Listed rather than reworded because dropping the `^` to slip past the detector is the one-rename evasion this guard's own header rejects, and it would weaken the assertion — the anchor is what proves the line starts a line
