...
skill-sources/refactor f02~an example schema migration over literal placeholder operands ({vocabulary.notes}, [default value]) under prose that reads Example: add a new required field. Until 2026-08-26 this fence also carried the only BSD-spelled in-place sed in the template tree, which exits 2 on GNU sed and succeeds on macOS; that is why the local dry run found four newly-visible failures and CI found five. It was rewritten to awk over a temp file, so the skip now rests only on the placeholder operands
