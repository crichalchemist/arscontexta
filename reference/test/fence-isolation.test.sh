...
skill-sources/refactor f02~an example schema migration over literal placeholder operands ({vocabulary.notes}, [default value]) under prose that reads Example: add a new required field. It also carries the only sed -i in the whole template tree, spelled BSD-style with an empty backup suffix: GNU sed reads that empty argument as the script and exits 2, so this fence fails on Linux and passes on macOS -- which is why the local dry run found four newly-visible failures and CI found five. Skipped as illustrative; the BSD-ism is recorded here rather than fixed, because a portable rewrite has to decide what happens to file modes and that is not this change
