...
```bash
awk '/^SCOPE="/{f=1;next} /^"/{f=0} f&&NF' reference/check-prose-paths.sh   # the 12 in scope
grep -c 'session-orient' reference/check-prose-paths.sh                    # 2: both now listed
```

**It became twelve on the branch that rewrote `.opencode/INSTALL.md` into a procedure
an agent executes.** That rewrite made the file dense with repo paths — `skills/`,
`reference/hosts/opencode-tools.md`, `.opencode/plugins/arscontexta.js` — so it was added
to SCOPE in the same change. Recorded here rather than silently corrected, per the
paragraph above; the count in the fence moved with it, and it is still ungated, so the
command remains the only thing to trust.
