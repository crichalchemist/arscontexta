...
- searches: [{type:"lex", query:"[{vocabulary.note}'s core concepts and mechanisms]"}, {type:"vec", query:"[{vocabulary.note}'s core concepts and mechanisms]"}]
- limit: 15
...
case "$QUEUE_FILE" in
  *.yaml)
    queue_yaml "$QUEUE_FILE" --where 'id={task_id}' \
      --set 'current_phase={vocabulary.reweave}' \
      --append 'completed_phases={vocabulary.reflect}'
    ;;
  *)
    queue_edit '(.tasks[] | select(.id==$id)).current_phase = "{vocabulary.reweave}" |
        (.tasks[] | select(.id==$id)).completed_phases += ["{vocabulary.reflect}"]' \
        "$QUEUE_FILE" --arg id "{task_id}"
    ;;
esac
```
