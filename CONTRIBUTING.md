...
for s in bash zsh; do
  ...
  $s reference/test/threshold-namespace.test.sh | tail -1   # expect: 56 passed, 0 failed
  ...
  $s reference/test/queue-edit.test.sh          | tail -1   # expect: passed=37 failed=0
done

./reference/validate-kernel.sh <your-vault>            # expect: every primitive PASSes
```

**`queue-edit.test.sh` is green: 37/37, both shells — all CI steps pass.** It was RED ON PURPOSE for
