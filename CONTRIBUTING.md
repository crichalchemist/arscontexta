...
for s in bash zsh; do
  ...
  $s reference/test/queue-edit.test.sh          | tail -1   # expect: passed=36 failed=0
done

./reference/validate-kernel.sh <your-vault>            # expect: every primitive PASSes
```

**`queue-edit.test.sh` is green: 36/36, both shells — all CI steps pass.** It was RED ON PURPOSE for
