...
3. **Query the research graph** -- use a hybrid `mcp__qmd__query` (lex+vec, LLM-reranked) to search for claims relevant to the friction. Fall back to a `vec`-only query with `rerank=false`. If MCP is unavailable, use qmd CLI (`qmd query`, then `qmd vsearch`). Fall back to reading bundled reference files directly only if both MCP and qmd CLI are unavailable.
