...
**Search strategy:** Use a hybrid `query` (lex+vec, LLM-reranked — highest quality) for conceptual questions. Use a `vec`-only `query` (`rerank=false`) for semantic exploration. Use a `lex`-only `query` (`rerank=false`) for known terminology. All searches use the `methodology` collection.
...
**Search strategy:** a `lex`-only `query` (`rerank=false`) with keywords from the question using the `methodology` collection. To narrow to guidance docs, add `kind:guidance` to your grep filter on results.
...
**Search strategy:** Use a `vec`-only `query` (`rerank=false`) across the `methodology` collection for semantic domain matching. To list all examples: `rg '^kind: example' ${CLAUDE_PLUGIN_ROOT}/methodology/`.
...
```
mcp__qmd__query  searches=[{type:"lex",query:"[user's question rephrased as a search]"},{type:"vec",query:"[user's question rephrased as a search]"}]  collections=["methodology"]  limit=10
```
Use a hybrid `query` (lex+vec, LLM-reranked) for conceptual questions because the best connections often use different vocabulary than the question. Results will include all kinds; prioritize `kind: research` results.

**For HOW questions (Guidance Docs):**
```
mcp__qmd__query  searches=[{type:"lex",query:"[key terms from question]"}]  collections=["methodology"]  limit=5  rerank=false
```
...
```
mcp__qmd__query  searches=[{type:"vec",query:"[domain + what the user wants to see]"}]  collections=["methodology"]  limit=5  rerank=false
```
...
**Search:**
1. `mcp__qmd__query  searches=[{type:"lex",query:"atomic notes vs compound documents granularity"},{type:"vec",query:"atomic notes vs compound documents granularity"}]  collections=["methodology"]  limit=8`
...
**Search:**
1. `mcp__qmd__query  searches=[{type:"lex",query:"large source processing chunking"}]  collections=["methodology"]  limit=5  rerank=false`
2. `mcp__qmd__query  searches=[{type:"lex",query:"context degradation large documents extraction"},{type:"vec",query:"context degradation large documents extraction"}]  collections=["methodology"]  limit=5`
...
**Search:**
1. `mcp__qmd__query  searches=[{type:"vec",query:"cooking recipes culinary knowledge system"}]  collections=["methodology"]  limit=5  rerank=false`
