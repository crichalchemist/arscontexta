...
# /reference/... that is never readable, and this block exited 1 on every vault
# in every state — no proposal could ever cite link evidence.
...
- Use a hybrid `query` (lex+vec, LLM-reranked) to find claims that address the specific friction
- Fall back to a `vec`-only `query` with `rerank=false` if reranking is unavailable
...
```
mcp__qmd__query  searches=[{type:"lex",query:"[friction description in natural language]"},{type:"vec",query:"[friction description in natural language]"}]  limit=10
```
